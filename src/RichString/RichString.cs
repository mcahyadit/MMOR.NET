...
public interface IRichStringFormatter {
  public StringBuilder Format(IRichString rich_str,
      StringBuilder? result);

  public string Format(IRichString rich_str) =>
      Format(rich_str, null).ToString();
}
...
public interface IRecursiveRichString : IRichString { 
  ...
  [Pure] IRichString IRichString.Replace(in string old_value, in string new_value) =>
      ReplaceString(str.Replace(old_value, new_value));

  [Pure] IRichString IRichString.Replace(char old_value, char new_value) =>
      ReplaceString(str.Replace(old_value, new_value));

  [Pure] IRichString IRichString.Remove(int start_index, int length) =>
...
public class RichStringBuilder : IRichString {
  ...
  public RichStringBuilder(params IRichString[] parts) =>
      components_ = parts.ToList();

  public RichStringBuilder(params string[] parts) => components_ =
      parts.Select(x => (IRichString)x.AsRichString()).ToList();

  public RichStringBuilder(RichStringBuilder copy) =>
      components_ = copy.components_.ToList();
...
#region Generic String API
  ...
  public bool IsNullOrEmpty() => components_.IsNullOrEmpty() ||
      components_.All(x => x.IsNullOrEmpty());

  public bool IsNullOrWhiteSpace() => components_.IsNullOrEmpty() ||
      components_.All(x => x.IsNullOrWhiteSpace());
  ...
  //-+-+-+-+-+-+-+-+
#region Operators
  public static RichStringBuilder operator +(RichStringBuilder left,
      RichStringBuilder right) {
  ...
  public static RichStringBuilder operator +(IRichString left,
      RichStringBuilder right) {
  ...
  public static RichStringBuilder operator +(string left,
      RichStringBuilder right) {
    var copy = new RichStringBuilder(right);
...
public static partial class RichStringUtils {
  ...
  public static string Format(this IRichString rich_str,
      in IRichStringFormatter formatter) =>
      formatter.Format(rich_str);

  public static RichStringBuilder PadSmart(this IRichString rich_str,
      int total_width, char padding_char = ' ') =>
      total_width < 0
          ? PadLeft(rich_str, -total_width, padding_char)
          : PadRight(rich_str, total_width, padding_char);

  public static RichStringBuilder PadLeft(this IRichString rich_str,
      int total_width, char padding_char = ' ') {
    var result = new RichStringBuilder(rich_str);
    result.Prepend(
        (RichStringPlain)new string(padding_char,
            Math.Max(0, total_width - rich_str.Length)));
    return result;
  }

  public static RichStringBuilder PadRight(this IRichString rich_str,
      int total_width, char padding_char = ' ') {
    var result = new RichStringBuilder(rich_str);
    result.Append(
        (RichStringPlain)new string(padding_char,
  ...
  public static RichStringBuilder PadCenter(this IRichString rich_str,
      int total_width, char padding_char = ' ') {
    var result = new RichStringBuilder(rich_str);
    int total = Math.Abs(total_width) - rich_str.Length;
    int left = total / 2;
    if (total_width < 0) total += 1;
  ...
  public static IList<IRichString> SplitByNewLine(this IRichString input) {
    ...
    void Process(IRichString rich) {
      switch (rich) {
        case RichStringPlain plain:
          ...
          for (var i = 0; i < len; ++i) {
            if (i > 0) EmitLine(); // newline was found
            if (lines[i].Length > 0)
    ...
    void EmitLine() {
      result.Add(currentLine.Components.Count == 0
          ? kRichEmpty
          : new RichStringBuilder(currentLine));
      currentLine.Clear();
    }
  }
