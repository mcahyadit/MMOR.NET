...
namespace MMOR.NET.Statistics
{
  ...
  public static class QuantileFinder
  {
    ...
    /// <returns></returns>
    public static double QuantileSorted<T>(this IReadOnlyList<T> list, double tau)
    ...
    public static double QuantileSorted<T>(this SortedDictionary<T, uint> map, double tau)
      where T : IConvertible
    {
      // Clamps tau
      if (tau > 1 || tau < 0)
      ...
      foreach (KeyValuePair<T, uint> kvp in map)
      {
        count += kvp.Value;
        if (count >= hf)
          top ??= kvp.Key.ToDouble(null);
        if (count >= hf - 1)
          bot ??= kvp.Key.ToDouble(null);
        if (top.HasValue && bot.HasValue)
          break;
      }
    ...
    /// </summary>
    public static double Quantile<T>(this IReadOnlyList<T> list, double tau)
      where T : IConvertible
    {
    ...
    public static double Quantile<T>(this IDictionary<T, uint> map, double tau)
      where T : IConvertible => QuantileSorted(map.ConvertToSortedDouble(), tau);
  }
}
