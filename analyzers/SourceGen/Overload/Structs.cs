...
public partial class OverloadGenerator {
  internal readonly record struct Typemap {
    ...
    public readonly string class_lib { get; init; }
  ...
  internal record struct MethodModel {
    ...
    internal EquatableArray<string> usings;
    ...
    internal EquatableArray<ParameterModel> parameters;

    public MethodModel(MethodDeclarationSyntax decl, IMethodSymbol method) {
      valid = true;

      asmname = method.ContainingAssembly.Name;
      HashSet<string> usings_tmp =
          [..SourceGenUtils.GetRootUsings(decl), "System.Runtime.CompilerServices"];

      usings = usings_tmp.ToEquatableArray();
      ...
      name        = method.Name;
      type_args =
          method.TypeParameters.Select(SourceGenUtils.TypeArgsToStringPair).ToEquatableArray();
